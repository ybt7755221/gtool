...
//查找单条数据
func(u *WrProjectsModel) FindOne(conditions *WrProjects) (*WrProjects, error) {
	dbConn := DB.GetDB(Gin)
	defer dbConn.Close()
...
//查找所有数据
func (u *WrProjectsModel) FindAll(conditions *WrProjects, sort map[string]string) ([]WrProjects, error) {
	wrProjectsList := new(WrProjectsPageDao).List
	...
	err := dbConn.Find(&wrProjectsList, conditions)
	defer dbConn.Close()
