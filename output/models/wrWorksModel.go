...
//查找单条数据
func(u *WrWorksModel) FindOne(conditions *WrWorks) (*WrWorks, error) {
	dbConn := DB.GetDB(Gin)
	defer dbConn.Close()
...
//查找所有数据
func (u *WrWorksModel) FindAll(conditions *WrWorks, sort map[string]string) ([]WrWorks, error) {
	wrWorksList := new(WrWorksPageDao).List
	...
	err := dbConn.Find(&wrWorksList, conditions)
	defer dbConn.Close()
